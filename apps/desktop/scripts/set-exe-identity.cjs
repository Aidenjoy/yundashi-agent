#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Yundashi Agent icon + version metadata onto the
// built Yundashi Agent.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Yundashi Agent.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.cjs <path-to-Yundashi Agent.exe>
//
...
// Stamp the Yundashi Agent icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Yundashi Agent',
      FileDescription: 'Yundashi Agent',
      CompanyName: '云大师',
      LegalCopyright: 'Copyright (c) 2026 云大师'
    }
  })

  console.log('[set-exe-identity] done — Yundashi Agent icon + identity stamped')
}
