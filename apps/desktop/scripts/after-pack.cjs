 ...
 *
 * Stamps the Yundashi Agent icon + identity onto the packed Windows Yundashi Agent.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Yundashi Agent')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Yundashi Agent'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Yundashi Agent.exe keeps the stock Electron icon`)
  }
}
