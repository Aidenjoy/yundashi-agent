...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Yundashi Agent & Nous-trained models',
    docsUrl: 'https://portal.yundashi.com',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Yundashi Agent needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Yundashi Agent stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Yundashi Agent can read from one file request.',
  approvals: {
    mode: 'How Yundashi Agent handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Yundashi Agent updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
